<?php 
$user = $conn->query("SELECT * FROM author_list where id ='".$_settings->userdata('id')."'");
				...
				<form action="" id="manage-user">	
					...
					<div class="row">
						<div class="form-group col-md-6">
							<label for="shop_name" class="control-label">Publisher Name</label>
							<input type="text" id="publisher_name" autofocus name="publisher_name" class="form-control form-control-sm form-control-border" value="<?= isset($publisher_name) ? $publisher_name : "" ?>" required>
						</div>
						<div class="form-group col-md-6">
							<label for="shop_owner" class="control-label">Publisher Owner Fullname</label>
							<input type="text" id="spublisher_owner" name="publisher_owner" class="form-control form-control-sm form-control-border" value="<?= isset($publisher_owner) ? $publisher_owner : "" ?>" required>
						</div>
						<div class="form-group col-md-6">
						...
						<div class="form-group col-md-6">
							<label for="publisher_type_id" class="control-label">Publisher Type</label>
							<select type="text" id="publisher_type_id" name="publisher_type_id" class="form-control form-control-sm form-control-border select2" required>
								<option value="" disabled selected></option>
								<?php 
								$types = $conn->query("SELECT * FROM `publisher_type_list` where delete_flag = 0 and `status` = 1 ".(isset($publisher_type_id) ? " or id = '{$publisher_type_id}' " : "")." order by `name` asc ");
								while($row = $types->fetch_assoc()):
								?>
								<option value="<?= $row['id'] ?>" <?= isset($publisher_type_id) && $publisher_type_id == $row['id'] ? 'selected' : '' ?>><?= $row['name'] ?></option>
								<?php endwhile; ?>
							</select>
						</div>
					</div>
					...
					<div class="row">
						<div class="form-group col-md-6">
							<label for="logo" class="control-label">Publisher Logo</label>
