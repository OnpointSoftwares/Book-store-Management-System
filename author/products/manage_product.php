...
<div class="container-fluid">
	<form action="" id="product-form">
		...
		<input type="hidden" name ="author_id" value="<?= $_settings->userdata('id') ?>">
		<div class="row">
			<div class="col-md-6">
				...
				<div class="form-group">
						...
						<select type="text" id="category_id" name="category_id" class="form-control form-control-sm form-control-border select2" required>
							...
							<?php 
							$categories = $conn->query("SELECT * FROM `category_list` where delete_flag = 0 and `status` = 1 and author_id= '{$_settings->userdata('id')}' ".(isset($category_id) ? " or id = '{$category_id}' " : '')." order by `name` asc ");
