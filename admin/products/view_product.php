...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT p.*, c.name as `category`,v.code, v.publisher_name as `vendor` from `product_list` p inner join category_list c on p.category_id = c.id inner join author_list v on p.author_id = v.id where p.id = '{$_GET['id']}' and p.delete_flag = 0 ");
    if($qry->num_rows > 0){
