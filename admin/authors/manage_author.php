<?php 
if(isset($_GET['id'])){
	$vendor = $conn->query("SELECT * FROM author_list where id ='".$_GET['id']."'");
	if($vendor->num_rows > 0){
		...
		echo '<script> alert("Unknown Vendor"); location.replace("./?page=authors")</script>';
	}
				...
				<form action="" id="manage-user">	
					...
					<div class="row">
						<div class="form-group col-md-6">
							...
							<input type="text" id="shop_name" autofocus name="shop_name" class="form-control form-control-sm form-control-border" value="<?= isset($shop_name) ? $publisher_name : "" ?>" required>
						</div>
						<div class="form-group col-md-6">
						...
						<div class="form-group col-md-6">
							<label for="shop_type_id" class="control-label">Publisher Type</label>
							<select type="text" id="shop_type_id" name="shop_type_id" class="form-control form-control-sm form-control-border select2" required>
								...
								<?php 
								$types = $conn->query("SELECT * FROM `publisher_type_list` where delete_flag = 0 and `status` = 1 ".(isset($shop_type_id) ? " or id = '{$shop_type_id}' " : "")." order by `name` asc ");
