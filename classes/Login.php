...
class Login extends DBConnection {
	...
	public function login_author(){
		extract($_POST);
		$password = md5($password);
		$stmt = $this->conn->prepare("SELECT * from author_list where username = ? and password =? and delete_flag = ?  ");
		$delete_flag = 0;
...
switch ($action) {
	...
	case 'logout_client':
		...
		break;
	case 'login_author':
		echo $auth->login_author();
		break;
	case 'logout_author':
		echo $auth->logout_author();
		break;
	default:
		echo $auth->index();
		break;
