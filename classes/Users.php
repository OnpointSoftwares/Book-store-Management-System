...
Class Users extends DBConnection {
	...
	public function save_author(){
		...
		if(empty($_POST['id'])){
			...
			$code = sprintf("%'.05d",1);
			while(true){
				$check = $this->conn->query("SELECT * FROM `author_list` where code = '{$prefix}{$code}'")->num_rows;
				if($check > 0){
		...
		if(isset($oldpassword) && !empty($id)){
			$current_pass = $this->conn->query("SELECT * FROM `author_list` where id = '{$id}'")->fetch_array()['password'];
		...
		$check  = $this->conn->query("SELECT * FROM `author_list` where username = '{$username}' and delete_flag = 0 ".(!empty($id) ? " and id !='{$id}'" : ''))->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = " Username already exists";
		}else{
			if(empty($id)){
				$sql = "INSERT INTO `author_list` set {$data}";
			}else{
				$sql = "UPDATE `author_list` set {$data} where id = '{$id}'";
			}
			...
			if($save){
				...
				if(empty($id)){
					if(strpos($_SERVER['HTTP_REFERER'], base_app.'author/register.php') > -1){
						$resp['msg'] = " Your account has been registered successfully.";
					}else{
						$resp['msg'] = " Author's Account has been registered successfully.";
					}
				}else{
					if($this->settings->userdata('login_type') == 2){
						$resp['msg'] = " Your account details has been updated successfully.";
					}else{
						$resp['msg'] = " Author's Account Details has been updated successfully.";
					}	
				}

				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					if(!is_dir(base_app."uploads/authors"))
					mkdir(base_app."uploads/authors");
					$fname = 'uploads/authors/'.($vid).'.png';
					$dir_path =base_app. $fname;
					$upload = $_FILES['img']['tmp_name'];
					$type = mime_content_type($upload);
					$allowed = array('image/png','image/jpeg');
					if(!in_array($type,$allowed)){
						...
						if($gdImg){
								...
								imagedestroy($t_image);
								if($uploaded_img){
									$qry = $this->conn->query("UPDATE `author_list` set avatar = concat('{$fname}','?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '$vid' ");
	...
	public function delete_author(){
		extract($_POST);
		$qry = $this->conn->query("UPDATE author_list set delete_flag = 1 where id = $id");
		if($qry){
			$this->settings->set_flashdata('success',' Author Details successfully deleted.');
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
...
switch ($action) {
	...
	break;
	case 'save_author':
		echo $users->save_author();
	break;
	case 'delete_author':
		echo $users->delete_author();
	break;
	case 'save_client':
