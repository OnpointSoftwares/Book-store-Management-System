...
Class Master extends DBConnection {
	...
	function save_publisher_type(){
		...
		$check = $this->conn->query("SELECT * FROM `publisher_type_list` where `name` = '{$name}' and delete_flag = 0 ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "publisher Type already exists.";
		}else{
			if(empty($id)){
				$sql = "INSERT INTO `publisher_type_list` set {$data} ";
			}else{
				$sql = "UPDATE `publisher_type_list` set {$data} where id = '{$id}' ";
			}
			$save = $this->conn->query($sql);
			if($save){
				$resp['status'] = 'success';
				if(empty($id))
				$resp['msg'] = " New Publisher Type successfully saved.";
				else
				$resp['msg'] = " Publisher Type successfully updated.";
			}else{
				$resp['status'] = 'failed';
	...
	function delete_publisher_type(){
		extract($_POST);
		$del = $this->conn->query("UPDATE `publisher_type_list` set delete_flag = 1 where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Publisher Type successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_category(){
		...
		$check = $this->conn->query("SELECT * FROM `category_list` where `name` = '{$name}' and author_id = '{$author_id}' and delete_flag = 0 ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function save_product(){
		...
		$check = $this->conn->query("SELECT * FROM `product_list` where author_id = '{$author_id}' and `name` = '{$name}' and delete_flag = 0 ".(!empty($id) ? " and id != '{$id}'" : ""))->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = ' Book Name Already exists.';
		}else{
			if(empty($id)){
			...
			if($save){
				...
				$resp['status'] = 'success';
				if(empty($id))
					$resp['msg'] = " New Book successfully saved.";
				else
	...
	function place_order(){
		...
		$has_failed=false;
		$gtotal = 0;
		$authors = $this->conn->query("SELECT * FROM `author_list` where id in (SELECT author_id from product_list where id in (SELECT product_id FROM `cart_list` where client_id ='{$this->settings->userdata('id')}')) order by `publisher_name` asc");
		$prefix = date('Ym-');
		$code = sprintf("%'.05d",1);
		while($vrow = $authors->fetch_assoc()):
			$data = "";
			while(true){
			...
			$sql = "INSERT INTO `order_list` (`code`,`client_id`,`author_id`,`delivery_address`) VALUES {$data}";
			$save = $this->conn->query($sql);
			if($save){
				...
				$data = "";
				$gtotal = 0 ;
				$products = $this->conn->query("SELECT c.*, p.name as `name`, p.price,p.image_path FROM `cart_list` c inner join product_list p on c.product_id = p.id where c.client_id = '{$this->settings->userdata('id')}' and p.author_id = '{$vrow['id']}' order by p.name asc");
...
switch ($action) {
	case 'save_publisher_type':
		echo $Master->save_publisher_type();
	break;
	case 'delete_publisher_type':
		echo $Master->delete_publisher_type();
	break;
	case 'save_category':
