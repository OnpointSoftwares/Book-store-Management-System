...
<div class="col-lg-12 py-5">
    <div class="contain-fluid">
        ...
        <h3 class="text-center"><b>Books</b></h3>
        <center><hr class="w-25"></center>
        <div class="row" id="product_list">
            <?php 
            $products = $conn->query("SELECT p.*, v.publisher_name as author, c.name as `category` FROM `product_list` p inner join author_list v on p.author_id = v.id inner join category_list c on p.category_id = c.id where p.delete_flag = 0 and p.`status` =1 order by RAND() limit 4");
        ...
        <div class="text-center">
            <a href="./?page=products" class="btn btn-large btn-primary rounded-pill col-lg-3 col-md-5 col-sm-12">Explore More Books</a>
        </div>
    </div>
</div>