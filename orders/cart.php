    ...
    <div class="card card-outline card-primary rounded-0 shadow-0">
        ...
        <div class="card-body">
            <div id="cart-list">
                ...
                <?php 
                $gtotal = 0;
                $authors = $conn->query("SELECT * FROM `author_list` where id in (SELECT author_id from product_list where id in (SELECT product_id FROM `cart_list` where client_id ='{$_settings->userdata('id')}')) order by `publisher_name` asc");
                while($vrow=$authors->fetch_assoc()):                
                ?>
                    <div class="col-12 border">
                        <span>Author: <b><?= $vrow['code']. " - " . $vrow['publisher_name'] ?></b></span>
                    </div>
                    <div class="col-12 border p-0">
                        <?php 
                        $vtotal = 0;
                        $products = $conn->query("SELECT c.*, p.name as `name`, p.price,p.image_path FROM `cart_list` c inner join product_list p on c.product_id = p.id where c.client_id = '{$_settings->userdata('id')}' and p.author_id = '{$vrow['id']}' order by p.name asc");
